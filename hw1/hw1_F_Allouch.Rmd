...
setwd("C:/Users/fallouch/Box/PhD/Spring 2022/Clustered & Longitudinal Data Analysis/hw/hw1")
...
## Question 4 (b)
```{r}
independent_lm <- lm(PHQ ~ Treatment,
                     data = dat_long)

summary(independent_lm)
```

Using a linear regression model with treatment as the predictor and PHQ as the outcome, assuming independence of outcomes, the coefficient estimate of treatment is -3.12, with a p-value < 0.001. This can be interpreted as, compared to the control group, treatment is associated with a -3.12 change in PHQ. The p-value of this estimate is less than our cutoff of 0.05, which gives us sufficient evidence to reject $H_0$, suggesting that treatment is significantly associated with PHQ.
...
#Copy the function panel.cor from the manual to add the correlation
# pairs(cbind(visit0, visit1, visit3, visit6, visit9, visit12),
